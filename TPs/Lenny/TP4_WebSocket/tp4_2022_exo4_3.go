...
func main() {
	...
	for true {
		...
		fmt.Println("b - View 20 recent messages and subscribe for new messages")
		fmt.Println("c - Quit")
		...
		case 'a':
			...
			jsonOfNewMessage, _ := json.Marshal(jsonMessage{Type: "post", Message: newMessage})
			stringOfJason := string(jsonOfNewMessage)

			errorMessage = connection.WriteMessage(1, []byte(stringOfJason)) // TextMessage = 1
			if errorMessage != nil {
