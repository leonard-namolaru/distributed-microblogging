...
const URL = "https://127.0.0.1:8443/udp-address.json" // https://jch.irif.fr:8443/udp-address.json , https://127.0.0.1:8443/udp-address.json
...
func main() {
	...
	for i := 0; responseCount < 2; i++ {
		...
		fmt.Printf("Read deadline = RTO : %d sec \n", int64(readDeadline))
		...
		if errorMessage != nil && int64(readDeadline) >= 60 {
			...
			if !allZero {
				...
				sample := time.Since(beginning).Seconds()
				delta := math.Abs(sample - RTT)
				RTT := ALPHA*RTT + (1.0-ALPHA)*sample
