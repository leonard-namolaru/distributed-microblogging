...
func getHttpResponse(client *http.Client, requestUrl string) ([]byte, int) {
...
func main() {
	...
	body, lastContentLength := getHttpResponse(client, CHAT_URL)
	...
	for i := forBeginning; i < len(ids); i++ {
		...
		body, _ = getHttpResponse(client, httpGetMessage)
	...
	for true {
		...
		body, contentLength := getHttpResponse(client, CHAT_URL)
