...
func main() {
	...
	for i := 0; i < 1 && !responseReceived; i++ {
		...
		if errorMessage != nil {
			log.Printf("Function connection.Write() %s : ", errorMessage)
		}

		buffer := make([]byte, 1500)
		// func (c *UDPConn) SetReadDeadline(t time.Time) error
		// SetReadDeadline implements the Conn SetReadDeadline method.
		errorMessage = connection.SetReadDeadline(time.Now().Add(2 * time.Second))
		if errorMessage != nil {
			log.Fatal("Function connection.SetReadDeadline() : ", errorMessage)
		}

		_, errorMessage = bufio.NewReader(connection).Read(buffer)
		if errorMessage != nil && i == 1 {
			log.Printf("Timeout !")
		} else {
			allZero := true
			for i := range buffer {
